...
export default function RecordDetailPage({
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-background pb-24">
        <div className="bg-card border-b-2 border-border h-16"></div>
  ...
  if (error || !record) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4">
        ...
        <h2 className="text-2xl font-black mb-2 text-foreground">Record Not Found</h2>
        <p className="text-muted-foreground mb-8 text-center font-medium max-w-sm">{error}</p>
        <Button onClick={() => router.push('/lookup')} className="h-14 px-8 text-[16px] font-extrabold rounded-xl">Back to Search</Button>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <div className="w-full bg-card border-b-2 border-border shadow-sm sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 h-16 flex items-center">
          <button onClick={() => router.push('/lookup')} className="flex items-center text-muted-foreground hover:text-foreground font-bold text-[15px] transition-colors">
            <ArrowLeft className="w-5 h-5 mr-2" /> Back to Ledger
          </button>
        </div>
      </div>

      <div className="max-w-lg mx-auto p-4 sm:p-6 space-y-6 mt-4">
        {/* Header Info */}
        <div className="flex justify-between items-end mb-2">
          <div>
            <p className="text-[12px] font-bold text-muted-foreground tracking-widest uppercase mb-1">Record Detail</p>
            <h2 className="text-[32px] font-black text-foreground leading-none flex items-center gap-3">
              Status: <span className={cn("text-[18px] mt-1.5", isOfflineRecord ? "text-amber-600" : "text-emerald-600")}>{isOfflineRecord ? 'Pending Sync' : 'Synced'}</span>
            </h2>
          </div>
          <div className="text-right bg-muted px-3 py-1.5 rounded-lg border border-border">
            <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Record ID</p>
            <p className="text-[14px] text-foreground font-mono font-black tracking-tight">{record.reference_code}</p>
          </div>
        </div>

        {isOfflineRecord && (
          <div className="p-4 bg-amber-50 text-amber-900 border-2 border-amber-200 rounded-2xl text-[15px] font-bold flex items-start gap-3 shadow-sm">
        ...
        <div className={cn(
          "rounded-2xl p-5 flex items-start gap-4 border-2 shadow-sm",
          record.is_override
          ...
          {record.is_override ? (
            ...
            <XCircle className="w-8 h-8 shrink-0 mt-0.5 text-rose-600" />
          )}
          ...
          <div className="flex-1">
            ...
            <p className="text-[15px] opacity-90 leading-snug mt-1 font-bold">
              {(() => {
            ...
            {record.is_override && (
               <p className="mt-3 text-[15px] italic opacity-90 border-t border-amber-200 pt-3">“{record.override_reason}”</p>
            )}
          </div>
        </div>

        {/* Inputs Recorded */}
        <div className="bg-card rounded-2xl border-2 border-input p-6 shadow-sm space-y-5">
          <div className="flex justify-between items-center border-b-2 border-border pb-3">
            <h3 className="text-[18px] font-black text-foreground uppercase tracking-tight">Inputs Recorded</h3>
          </div>
          
          <div className="grid grid-cols-2 gap-y-6 gap-x-4">
            <div>
              <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5">Volume</p>
              <p className="text-[20px] font-black text-foreground tracking-tight">{record.can_volume !== null && record.can_volume !== undefined ? `${record.can_volume.toFixed(1)} L` : '--'}</p>
            </div>
            <div>
              <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5">Fat</p>
              <p className="text-[20px] font-black text-foreground tracking-tight">{record.fat_percent?.toFixed(2)}%</p>
            </div>
            <div>
              <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5">SNF</p>
              <p className="text-[20px] font-black text-foreground tracking-tight">{record.snf_percent?.toFixed(2)}%</p>
            </div>
            <div>
              <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5">Temp</p>
              <p className="text-[20px] font-black text-foreground tracking-tight">{record.temperature?.toFixed(1)} &deg;C</p>
            </div>
            <div className="col-span-2 pt-2 border-t-2 border-border/50">
              <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5 mt-2">Adulteration Strip</p>
              <p className={cn("text-[20px] font-black tracking-tight", record.adulteration_result ? "text-rose-600" : "text-emerald-600")}>
        ...
        {record.evidence_type && (
          <div className="bg-card rounded-2xl border-2 border-input p-6 shadow-sm space-y-4">
            <h3 className="text-[18px] font-black text-foreground uppercase tracking-tight border-b-2 border-border pb-3">Evidence</h3>
            {record.evidence_type === 'photo' && record.photo_url && (
              <div className="space-y-3">
                <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest">Attached Photo</p>
            ...
            {record.evidence_type === 'sensory' && record.sensory_note && (
              <div className="space-y-3">
                <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest">Inspector Note</p>
                <p className="text-[16px] text-foreground font-bold bg-muted p-4 rounded-xl border border-border italic">
                  “{record.sensory_note}”
                </p>
              </div>
            )}
        ...
        {/* Farmer & Operator */}
        <div className="bg-card rounded-2xl border-2 border-input p-6 shadow-sm space-y-5">
          <h3 className="text-[18px] font-black text-foreground uppercase tracking-tight border-b-2 border-border pb-3">Farmer &amp; Operator</h3>
          <div>
            <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5">Farmer</p>
            <p className="text-[18px] font-black text-foreground tracking-tight">#{record.farmer_id.substring(0,6)}, {record.farmer?.name}</p>
          </div>
          <div>
            <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5">Timestamp</p>
            <p className="text-[16px] font-bold text-foreground">{format(dateObj, 'dd MMM yyyy, hh:mm a')}</p>
          </div>
          <div>
            <p className="text-[12px] text-muted-foreground font-bold uppercase tracking-widest mb-1.5">Operator ID</p>
            <p className="text-[16px] font-bold text-foreground tracking-tight">{record.operator_id.substring(0,8).toUpperCase()} {record.operator?.name ? `(${record.operator.name})` : ''}</p>
          </div>
        </div>

        {record.decision === 'rejected' && (
          <div className="pt-2 pb-2 space-y-5">
            <div className="bg-card rounded-2xl border-2 border-input p-6 shadow-sm flex flex-col items-center justify-center">
              <QRCodeCanvas 
                 ...
                 includeMargin={false}
              />
              <p className="mt-5 text-[12px] font-bold text-muted-foreground uppercase tracking-widest text-center">
            ...
            {!disputeStatus && !isOfflineRecord && (
              <Button
                ...
                className="w-full h-16 text-[18px] font-extrabold rounded-2xl bg-slate-900 hover:bg-slate-800 text-white shadow-md active:scale-[0.98] transition-all"
              >
                Dispute this Result
              </Button>
            )}
            ...
            {disputeStatus && (
              <div className={cn(
                "border-2 rounded-2xl p-5 flex gap-4 shadow-sm",
                disputeStatus.status === 'resolved' 
                  ? (disputeStatus.resolution_type === 'adjustment' ? "bg-emerald-50 border-emerald-200 text-emerald-900" : "bg-rose-50 border-rose-200 text-rose-900")
                  : "bg-blue-50 border-blue-200 text-blue-900"
              )}>
                {disputeStatus.status === 'resolved' ? (
                  disputeStatus.resolution_type === 'adjustment' ? <CheckCircle2 className="w-8 h-8 shrink-0 mt-0.5" /> : <XCircle className="w-8 h-8 shrink-0 mt-0.5" />
                ) : (
                  <AlertCircle className="w-8 h-8 shrink-0 mt-0.5" />
                )}
                <div>
                  <p className="font-black text-[16px] uppercase tracking-tight">
                    {disputeStatus.status === 'resolved' 
                      ? `Dispute Resolved: ${disputeStatus.resolution_type === 'adjustment' ? 'Accepted' : 'Rejected'}`
                      : 'Dispute already submitted'}
                  </p>
                  <p className="text-[14px] mt-1 opacity-90 font-medium">
                    {disputeStatus.status === 'resolved'
            ...
            >
              <QrCode className="w-6 h-6 mr-2" /> View Rejection Slip
            </Button>
          </div>
        )}
        ...
        <div className="mt-8 border-t-2 border-border pt-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-black text-[20px] text-foreground tracking-tight">Amendments</h3>
            {!isOfflineRecord && (
              <button 
                onClick={handleOpenCorrection} 
                className="flex items-center text-primary font-bold text-[14px] hover:underline"
              >
          ...
          {corrections.length === 0 ? (
            <div className="p-8 text-center border-2 border-dashed border-input rounded-2xl bg-card text-muted-foreground">
              <p className="font-bold text-[16px] text-foreground mb-1">No amendments</p>
              <p className="text-[14px] font-medium">This original record has not been corrected.</p>
            </div>
          ) : (
             <div className="space-y-6">
               {corrections.map((corr, idx) => (
                 <div key={corr.id} className="relative">
                    {idx === 0 && <div className="absolute -top-4 left-6 w-0.5 h-4 bg-border"></div>}
                    <div className="absolute -bottom-6 left-6 w-0.5 h-6 bg-border last:hidden"></div>
                    
                    <div className="border-2 border-blue-200 rounded-2xl bg-card shadow-sm relative z-10 overflow-hidden">
                      <div className="bg-blue-50 px-5 py-4 border-b-2 border-blue-100 flex items-center justify-between">
                         <div className="flex items-center gap-2">
                           <FileEdit className="w-5 h-5 text-blue-600" />
                           <span className="text-[13px] font-black text-blue-900 uppercase tracking-widest">
                             AMENDMENT
                           </span>
                         </div>
                      ...
                      <div className="p-5 space-y-4">
                         <div>
                           <span className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Corrected By</span>
                           <span className="font-bold text-[16px] text-foreground">{corr.operator?.name}</span>
                         </div>
                         <div>
                           <span className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Reason for Amendment</span>
                           <span className="font-bold text-[16px] text-foreground italic">“{corr.reason}”</span>
                         </div>
                         <div className="pt-4 border-t-2 border-border mt-2">
                           <span className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest block mb-3">Changed Values</span>
                           <div className="grid grid-cols-2 gap-3">
                              {Object.entries(corr.new_values).map(([key, value]) => {
                                 const oldVal = corr.old_values[key]
                                 if (oldVal === value) return null
                                 return (
                                   <div key={key} className="col-span-2 sm:col-span-1 bg-muted p-3 rounded-xl border border-border font-mono">
                                      <div className="text-[11px] text-muted-foreground font-sans font-bold uppercase mb-1">{key.replace('_', ' ')}</div>
                                      <div className="flex items-center gap-3">
                                        <span className="line-through text-muted-foreground text-[15px] font-bold">{String(oldVal)}</span>
                                        <ArrowRight className="w-4 h-4 text-blue-500" />
                                        <span className="font-black text-blue-700 text-[15px]">{String(value)}</span>
                                      </div>
                                   </div>
                                 )
                              })}
        ...
        {batchHistory.length > 0 && (
          <div className="mt-8 border-t-2 border-border pt-8">
             <h3 className="font-black text-[20px] text-foreground tracking-tight mb-5">Recent Records</h3>
             <div className="space-y-3">
               {batchHistory.map(entry => (
                 <div key={entry.reference_code} className="bg-card border-2 border-input p-4 rounded-xl shadow-sm flex items-center justify-between">
                   <div>
                     <p className="font-black text-[15px] text-foreground font-mono tracking-tight">{entry.reference_code}</p>
                     <p className="text-[13px] text-muted-foreground font-bold">{format(new Date(entry.test_performed_at), 'MMM d, h:mm a')}</p>
                   </div>
                   <div className="text-right flex flex-col items-end">
                     <span className={cn(
                       "px-3 py-1.5 text-[12px] font-black rounded-lg uppercase tracking-widest",
                       entry.decision === 'accepted' ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"
                     )}>
                       {entry.decision}
                     </span>
                     {entry.decision === 'rejected' && entry.reason_codes && entry.reason_codes.length > 0 && (
                       <span className="text-[12px] font-bold text-muted-foreground mt-1 max-w-[150px] truncate">
